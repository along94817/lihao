$(function(){
	...
	function moveHbg(){
		$(document).scroll(function(){
			...
			$('#BgImage').css({'background-position':'0px '+-a/2+'px'});
		});
	}
	moveHbg();
	...
	$('.mainImage-1').mouseenter(function(){
		var nextImg=$(this).index();
		var nextImgeLeft=$('.nextImages>li').eq(nextImg).position().left;
		$(this).children('.mainImageTu').children('img').css({'transform':'scale(1.1)'});
		$(this).children('p').animate({'left':'40px'});
		$('.nextImages li').eq(nextImg).css('margin-left','-10px');
	});
	$('.mainImage-1').mouseleave(function(){
		var nextImg=$(this).index();
		var nextImgeLeft=$('.nextImages>li').eq(nextImg).position().left;
		$(this).children('.mainImageTu').children('img').css({'transform':''});
		$(this).children('p').animate({'left':'30'});
		$('.nextImages li').eq(nextImg).css('margin-left','0px');
	});
	//鼠标单击事件
	function dbclick(){
